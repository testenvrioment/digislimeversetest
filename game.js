 ...
 * EXTENSIONS
 ***********************************************************************/
/* aiController_V2 by LeonE */

function ai_controller(obj, patrol_dist) {
    if (typeof obj.ai_state !== "string") obj.ai_state = "patrol"; 
    if (typeof obj.patrol_start_x !== "number") obj.patrol_start_x = obj.x;
    if (typeof obj.patrol_start_y !== "number") obj.patrol_start_y = obj.y;
    if (typeof obj.image_angle !== "number") obj.image_angle = 0;
    if (typeof obj.min_distance_player !== "number") obj.min_distance_player = 3;

    if (typeof obj.patrol_points !== "object" || obj.patrol_points.length === 0) {
        obj.patrol_points = [
            [obj.patrol_start_x - patrol_dist, obj.patrol_start_y - patrol_dist],
            [obj.patrol_start_x + patrol_dist, obj.patrol_start_y - patrol_dist],
            [obj.patrol_start_x + patrol_dist, obj.patrol_start_y + patrol_dist],
            [obj.patrol_start_x - patrol_dist, obj.patrol_start_y + patrol_dist]
        ];
    }
    if (typeof obj.patrol_index !== "number") obj.patrol_index = 0;
    if (typeof obj.ai_speed_patrol !== "number") obj.ai_speed_patrol = 1.5;
    if (typeof obj.ai_speed_chase !== "number") obj.ai_speed_chase = 2.5;
    if (typeof obj.ai_detect_range !== "number") obj.ai_detect_range = 250;
    ...
    function lineBlocked(x1, y1, x2, y2){
        ...
        for(var i=0;i<=steps;i++){
            ...
            var px = x1 + (x2 - x1)*t;
            var py = y1 + (y2 - y1)*t;
            if(instance_position(px, py, obj_block)) return true;
        }
        return false;
    }

    function moveTowards(obj, tx, ty, spd, min_dist){
        if(typeof min_dist === "undefined") min_dist = 0;
        var d = dist(obj.x, obj.y, tx, ty);
        if(d <= min_dist) return false;

        var angle = point_direction(obj.x, obj.y, tx, ty);
        var rad = angle * Math.PI / 180;
        var nx = obj.x + Math.cos(rad)*spd;
        var ny = obj.y - Math.sin(rad)*spd;

        nx = Math.max(0, Math.min(nx, room_width));
        ny = Math.max(0, Math.min(ny, room_height));

        if(!instance_position(nx, ny, obj_block)){
            obj.x = nx; obj.y = ny;
            return true;
        }
        return false;
    ...
    // 1. Check helpers first
    if(global.helpers && global.helpers.length>0){
        for(var i=0;i<global.helpers.length;i++){
            var h = global.helpers[i];
            var d = dist(obj.x, obj.y, h.x, h.y);
            if(d < minDist && !lineBlocked(obj.x, obj.y, h.x, h.y)){
                minDist = d;
                target = h;
            }
        }
    ...
    // 2. Check player if no helper
    if(!target && global.player){
        var d = dist(obj.x, obj.y, global.player.x, global.player.y);
        if(d < minDist && !lineBlocked(obj.x, obj.y, global.player.x, global.player.y)){
            minDist = d;
            target = global.player;
        }
    }

    obj.target_enemy = target;

    // --- STATE MACHINE ---
    if(obj.target_enemy){
        obj.ai_state = "chase";
        moveTowards(obj, obj.target_enemy.x, obj.target_enemy.y, obj.ai_speed_chase, obj.min_distance_player);
        obj.image_angle = point_direction(obj.x, obj.y, obj.target_enemy.x, obj.target_enemy.y);

        if(dist(obj.x, obj.y, obj.target_enemy.x, obj.target_enemy.y) < obj.min_distance_player){
            if(typeof obj.target_enemy.takeDamage === "function") obj.target_enemy.takeDamage(1);
        }
    } else {
        obj.ai_state = "patrol";
        var ptarget = obj.patrol_points[obj.patrol_index];
        if(!lineBlocked(obj.x, obj.y, ptarget[0], ptarget[1])){
            moveTowards(obj, ptarget[0], ptarget[1], obj.ai_speed_patrol);
        }
        if(dist(obj.x, obj.y, ptarget[0], ptarget[1]) < 1){
            obj.patrol_index = (obj.patrol_index+1)%obj.patrol_points.length;
        }
        obj.image_angle = point_direction(obj.x, obj.y, ptarget[0], ptarget[1]);
    }
}
...
/* aiHelper_V1 by LeonE */

function helper_controller(obj,Dist_From_Plr) {
    // --- Initialization ---
    if (typeof obj.target_enemy !== "object") obj.target_enemy = null;
    if (typeof obj.follow_distance !== "number") obj.follow_distance = Dist_From_Plr; // <-- set to 45 px
    if (typeof obj.speed_follow !== "number") obj.speed_follow = 2;
    if (typeof obj.speed_attack !== "number") obj.speed_attack = 3;
    if (typeof obj.attack_range !== "number") obj.attack_range = 25;
    if (typeof obj.image_angle !== "number") obj.image_angle = 0;
    // --- Helpers ---
    function dist(ax, ay, bx, by){ return Math.hypot(bx-ax, by-ay); }
    function lineBlocked(x1, y1, x2, y2){
    ...
    function moveTowards(obj, tx, ty, spd, min_dist){
        ...
        if(!instance_position(nx, ny, obj_block)){
            ...
            // Try slight angle offsets
            var offsets = [15,-15,30,-30,45,-45];
            for(var i=0;i<offsets.length;i++){
                var a = (angle + offsets[i]) * Math.PI / 180;
                nx = obj.x + Math.cos(a)*spd;
                ny = obj.y - Math.sin(a)*spd;
                nx = Math.max(0, Math.min(nx, room_width));
                ny = Math.max(0, Math.min(ny, room_height));
                if(!instance_position(nx, ny, obj_block)){
                    obj.x = nx; obj.y = ny;
    ...
    var closest = null;
    var minDist = Infinity;
    instance_foreach(function(e){
        if(e!==obj && e!==player && e.ai_state!==undefined){
            var d = dist(obj.x, obj.y, e.x, e.y);
            if(d < minDist){
                minDist = d;
                closest = e;
            }
    ...
    obj.target_enemy = closest;
    // --- Behavior ---
    if(obj.target_enemy && !lineBlocked(obj.x, obj.y, obj.target_enemy.x, obj.target_enemy.y)){
        // Attack enemy
        moveTowards(obj, obj.target_enemy.x, obj.target_enemy.y, obj.speed_attack, obj.attack_range);
        obj.image_angle = point_direction(obj.x, obj.y, obj.target_enemy.x, obj.target_enemy.y);
    } else {
        // Follow player at follow_distance
        var targetX = player.x;
        var targetY = player.y;
        // Keep exact follow distance
        var d = dist(obj.x, obj.y, targetX, targetY);
        if(d > obj.follow_distance){
            moveTowards(obj, targetX, targetY, obj.speed_follow, obj.follow_distance);
        } else if(d < obj.follow_distance - 2){ // prevent overlapping
            // Move slightly away to maintain distance
            var angle = point_direction(player.x, player.y, obj.x, obj.y);
            var rad = angle * Math.PI / 180;
            var nx = obj.x + Math.cos(rad)*obj.speed_follow;
            var ny = obj.y - Math.sin(rad)*obj.speed_follow;
            if(!instance_position(nx, ny, obj_block)){
                obj.x = nx; obj.y = ny;
            }
        }
        obj.image_angle = point_direction(obj.x, obj.y, targetX, targetY);
    }
}
...
/* removeWatermark by leonE */

(function() {
  // Wait until the DOM is ready
  document.addEventListener('DOMContentLoaded', function() {
    // Remove the Tululoo watermark link, if it exists
    document.querySelectorAll('a[href*="tululoo.com"]').forEach(el => el.remove());
    // Find the canvas element
    const canvas = document.getElementById('tululoocanvas');
    if (!canvas) return;
    ...
    // Run once and on every window resize
    resizeCanvas();
...
function __hb_Outer() { 
__sprite_init__(this, hb_Outer, 145, 145, 10, 49, 'Box', 10, 10, 120, 49, 65, ['img/hb_Outer_0.png']);
}; var hb_Outer = new __hb_Outer();

function __hb_Inner() { 
__sprite_init__(this, hb_Inner, 145, 145, 18, 37, 'Box', 10, 18, 120, 37, 50, ['img/hb_Inner_0.png']);
}; var hb_Inner = new __hb_Inner();

function __spr_Glow() { 
__sprite_init__(this, spr_Glow, 16, 16, 0, 0, 'Box', 16, 0, 32, 0, 32, ['img/spr_Glow_0.png','img/spr_Glow_1.png','img/spr_Glow_2.png','img/spr_Glow_3.png','img/spr_Glow_4.png','img/spr_Glow_5.png','img/spr_Glow_6.png','img/spr_Glow_7.png','img/spr_Glow_8.png','img/spr_Glow_9.png','img/spr_Glow_10.png','img/spr_Glow_11.png','img/spr_Glow_12.png','img/spr_Glow_13.png','img/spr_Glow_14.png','img/spr_Glow_15.png','img/spr_Glow_16.png','img/spr_Glow_17.png','img/spr_Glow_18.png','img/spr_Glow_19.png','img/spr_Glow_20.png','img/spr_Glow_21.png','img/spr_Glow_22.png','img/spr_Glow_23.png','img/spr_Glow_24.png','img/spr_Glow_25.png','img/spr_Glow_26.png','img/spr_Glow_27.png','img/spr_Glow_28.png','img/spr_Glow_29.png']);
}; var spr_Glow = new __spr_Glow();

function __spr_SlimeBllue() { 
__sprite_init__(this, spr_SlimeBllue, 24, 28, 12, 14, 'Box', 6, 0, 24, 0, 28, ['img/spr_SlimeBllue_0.png','img/spr_SlimeBllue_1.png','img/spr_SlimeBllue_2.png','img/spr_SlimeBllue_3.png','img/spr_SlimeBllue_4.png','img/spr_SlimeBllue_5.png','img/spr_SlimeBllue_6.png','img/spr_SlimeBllue_7.png','img/spr_SlimeBllue_8.png','img/spr_SlimeBllue_9.png','img/spr_SlimeBllue_10.png','img/spr_SlimeBllue_11.png','img/spr_SlimeBllue_12.png','img/spr_SlimeBllue_13.png','img/spr_SlimeBllue_14.png','img/spr_SlimeBllue_15.png']);
}; var spr_SlimeBllue = new __spr_SlimeBllue();

function __spr_MushRoom() { 
__sprite_init__(this, spr_MushRoom, 36, 42, 18, 21, 'Box', 9, 0, 36, 0, 42, ['img/spr_MushRoom_0.png','img/spr_MushRoom_1.png','img/spr_MushRoom_2.png','img/spr_MushRoom_3.png','img/spr_MushRoom_4.png','img/spr_MushRoom_5.png','img/spr_MushRoom_6.png','img/spr_MushRoom_7.png','img/spr_MushRoom_8.png','img/spr_MushRoom_9.png','img/spr_MushRoom_10.png']);
}; var spr_MushRoom = new __spr_MushRoom();

function __spr_MiniSlime() { 
__sprite_init__(this, spr_MiniSlime, 24, 27, 12, 13, 'Box', 4, 0, 24, 0, 27, ['img/spr_MiniSlime_0.png','img/spr_MiniSlime_1.png','img/spr_MiniSlime_2.png','img/spr_MiniSlime_3.png','img/spr_MiniSlime_4.png','img/spr_MiniSlime_5.png','img/spr_MiniSlime_6.png','img/spr_MiniSlime_7.png','img/spr_MiniSlime_8.png','img/spr_MiniSlime_9.png','img/spr_MiniSlime_10.png','img/spr_MiniSlime_11.png','img/spr_MiniSlime_12.png','img/spr_MiniSlime_13.png','img/spr_MiniSlime_14.png','img/spr_MiniSlime_15.png']);
}; var spr_MiniSlime = new __spr_MiniSlime();

function __spr_seedLing() { 
__sprite_init__(this, spr_seedLing, 12, 21, 6, 10, 'Box', 6, 0, 12, 0, 21, ['img/spr_seedLing_0.png','img/spr_seedLing_1.png','img/spr_seedLing_2.png','img/spr_seedLing_3.png','img/spr_seedLing_4.png','img/spr_seedLing_5.png','img/spr_seedLing_6.png','img/spr_seedLing_7.png','img/spr_seedLing_8.png','img/spr_seedLing_9.png','img/spr_seedLing_10.png']);
}; var spr_seedLing = new __spr_seedLing();

function __spr_purpSlime() { 
__sprite_init__(this, spr_purpSlime, 24, 28, 12, 14, 'Box', 10, 0, 24, 0, 28, ['img/spr_purpSlime_0.png','img/spr_purpSlime_1.png','img/spr_purpSlime_2.png','img/spr_purpSlime_3.png','img/spr_purpSlime_4.png','img/spr_purpSlime_5.png','img/spr_purpSlime_6.png','img/spr_purpSlime_7.png','img/spr_purpSlime_8.png','img/spr_purpSlime_9.png','img/spr_purpSlime_10.png','img/spr_purpSlime_11.png','img/spr_purpSlime_12.png','img/spr_purpSlime_13.png','img/spr_purpSlime_14.png','img/spr_purpSlime_15.png']);
}; var spr_purpSlime = new __spr_purpSlime();

function __spr_MinAqua() { 
__sprite_init__(this, spr_MinAqua, 16, 18, 8, 9, 'Box', 10, 0, 16, 0, 18, ['img/spr_MinAqua_0.png','img/spr_MinAqua_1.png','img/spr_MinAqua_2.png','img/spr_MinAqua_3.png','img/spr_MinAqua_4.png','img/spr_MinAqua_5.png','img/spr_MinAqua_6.png','img/spr_MinAqua_7.png','img/spr_MinAqua_8.png','img/spr_MinAqua_9.png','img/spr_MinAqua_10.png','img/spr_MinAqua_11.png','img/spr_MinAqua_12.png','img/spr_MinAqua_13.png','img/spr_MinAqua_14.png','img/spr_MinAqua_15.png']);
}; var spr_MinAqua = new __spr_MinAqua();

function __spr_MiniBliue() { 
__sprite_init__(this, spr_MiniBliue, 16, 18, 8, 9, 'Box', 10, 0, 16, 0, 18, ['img/spr_MiniBliue_0.png','img/spr_MiniBliue_1.png','img/spr_MiniBliue_2.png','img/spr_MiniBliue_3.png','img/spr_MiniBliue_4.png','img/spr_MiniBliue_5.png','img/spr_MiniBliue_6.png','img/spr_MiniBliue_7.png','img/spr_MiniBliue_8.png','img/spr_MiniBliue_9.png','img/spr_MiniBliue_10.png','img/spr_MiniBliue_11.png','img/spr_MiniBliue_12.png','img/spr_MiniBliue_13.png','img/spr_MiniBliue_14.png','img/spr_MiniBliue_15.png']);
}; var spr_MiniBliue = new __spr_MiniBliue();

function __spr_ExplosionFx() { 
__sprite_init__(this, spr_ExplosionFx, 256, 256, 128, 128, 'Box', 128, 0, 256, 0, 256, ['img/spr_ExplosionFx_0.png','img/spr_ExplosionFx_1.png','img/spr_ExplosionFx_2.png','img/spr_ExplosionFx_3.png','img/spr_ExplosionFx_4.png','img/spr_ExplosionFx_5.png','img/spr_ExplosionFx_6.png','img/spr_ExplosionFx_7.png','img/spr_ExplosionFx_8.png','img/spr_ExplosionFx_9.png','img/spr_ExplosionFx_10.png','img/spr_ExplosionFx_11.png','img/spr_ExplosionFx_12.png','img/spr_ExplosionFx_13.png','img/spr_ExplosionFx_14.png','img/spr_ExplosionFx_15.png']);
}; var spr_ExplosionFx = new __spr_ExplosionFx();

function __spr_MagRING() { 
__sprite_init__(this, spr_MagRING, 128, 128, 64, 64, 'Box', 64, 0, 128, 0, 128, ['img/spr_MagRING_0.png','img/spr_MagRING_1.png','img/spr_MagRING_2.png','img/spr_MagRING_3.png','img/spr_MagRING_4.png','img/spr_MagRING_5.png','img/spr_MagRING_6.png']);
}; var spr_MagRING = new __spr_MagRING();

function __spr_Burn() { 
__sprite_init__(this, spr_Burn, 128, 128, 64, 64, 'Box', 64, 0, 128, 0, 128, ['img/spr_Burn_0.png','img/spr_Burn_1.png','img/spr_Burn_2.png','img/spr_Burn_3.png','img/spr_Burn_4.png','img/spr_Burn_5.png','img/spr_Burn_6.png','img/spr_Burn_7.png','img/spr_Burn_8.png','img/spr_Burn_9.png','img/spr_Burn_10.png','img/spr_Burn_11.png','img/spr_Burn_12.png','img/spr_Burn_13.png','img/spr_Burn_14.png','img/spr_Burn_15.png','img/spr_Burn_16.png','img/spr_Burn_17.png','img/spr_Burn_18.png','img/spr_Burn_19.png','img/spr_Burn_20.png','img/spr_Burn_21.png','img/spr_Burn_22.png','img/spr_Burn_23.png','img/spr_Burn_24.png','img/spr_Burn_25.png','img/spr_Burn_26.png','img/spr_Burn_27.png','img/spr_Burn_28.png','img/spr_Burn_29.png','img/spr_Burn_30.png','img/spr_Burn_31.png','img/spr_Burn_32.png','img/spr_Burn_33.png','img/spr_Burn_34.png','img/spr_Burn_35.png','img/spr_Burn_36.png','img/spr_Burn_37.png','img/spr_Burn_38.png','img/spr_Burn_39.png','img/spr_Burn_40.png','img/spr_Burn_41.png','img/spr_Burn_42.png','img/spr_Burn_43.png','img/spr_Burn_44.png','img/spr_Burn_45.png','img/spr_Burn_46.png','img/spr_Burn_47.png','img/spr_Burn_48.png','img/spr_Burn_49.png','img/spr_Burn_50.png','img/spr_Burn_51.png','img/spr_Burn_52.png','img/spr_Burn_53.png','img/spr_Burn_54.png','img/spr_Burn_55.png','img/spr_Burn_56.png','img/spr_Burn_57.png','img/spr_Burn_58.png','img/spr_Burn_59.png','img/spr_Burn_60.png','img/spr_Burn_61.png','img/spr_Burn_62.png','img/spr_Burn_63.png']);
}; var spr_Burn = new __spr_Burn();

function __spr_Lightning() { 
__sprite_init__(this, spr_Lightning, 128, 128, 64, 64, 'Box', 64, 0, 128, 0, 128, ['img/spr_Lightning_0.png','img/spr_Lightning_1.png','img/spr_Lightning_2.png','img/spr_Lightning_3.png','img/spr_Lightning_4.png','img/spr_Lightning_5.png','img/spr_Lightning_6.png','img/spr_Lightning_7.png','img/spr_Lightning_8.png','img/spr_Lightning_9.png','img/spr_Lightning_10.png','img/spr_Lightning_11.png','img/spr_Lightning_12.png','img/spr_Lightning_13.png','img/spr_Lightning_14.png','img/spr_Lightning_15.png','img/spr_Lightning_16.png','img/spr_Lightning_17.png','img/spr_Lightning_18.png','img/spr_Lightning_19.png','img/spr_Lightning_20.png','img/spr_Lightning_21.png','img/spr_Lightning_22.png','img/spr_Lightning_23.png','img/spr_Lightning_24.png','img/spr_Lightning_25.png','img/spr_Lightning_26.png','img/spr_Lightning_27.png','img/spr_Lightning_28.png','img/spr_Lightning_29.png','img/spr_Lightning_30.png','img/spr_Lightning_31.png','img/spr_Lightning_32.png','img/spr_Lightning_33.png','img/spr_Lightning_34.png','img/spr_Lightning_35.png','img/spr_Lightning_36.png','img/spr_Lightning_37.png','img/spr_Lightning_38.png','img/spr_Lightning_39.png','img/spr_Lightning_40.png','img/spr_Lightning_41.png','img/spr_Lightning_42.png','img/spr_Lightning_43.png','img/spr_Lightning_44.png','img/spr_Lightning_45.png','img/spr_Lightning_46.png','img/spr_Lightning_47.png','img/spr_Lightning_48.png','img/spr_Lightning_49.png','img/spr_Lightning_50.png','img/spr_Lightning_51.png','img/spr_Lightning_52.png','img/spr_Lightning_53.png','img/spr_Lightning_54.png','img/spr_Lightning_55.png','img/spr_Lightning_56.png','img/spr_Lightning_57.png','img/spr_Lightning_58.png','img/spr_Lightning_59.png','img/spr_Lightning_60.png','img/spr_Lightning_61.png','img/spr_Lightning_62.png','img/spr_Lightning_63.png']);
}; var spr_Lightning = new __spr_Lightning();
...
function __snd_SlimeBite() { 
__audio_init__(this, snd_SlimeBite, 'aud/Bite_01.wav', '', '');
}; var snd_SlimeBite = new __snd_SlimeBite();

function __snd_Happy() { 
__audio_init__(this, snd_Happy, 'aud/fire_happymix_01.wav', '', '');
}; var snd_Happy = new __snd_Happy();

function __snd_RefreshedSlime4() { 
__audio_init__(this, snd_RefreshedSlime4, 'aud/refreshed_04.wav', '', '');
}; var snd_RefreshedSlime4 = new __snd_RefreshedSlime4();

function __snd_TransFormGood() { 
__audio_init__(this, snd_TransFormGood, 'aud/largo_transform_good_01.wav', '', '');
}; var snd_TransFormGood = new __snd_TransFormGood();
...
__instance_init__(this, obj_player, null, 1, 0, spr_SlimeBllue, 1, 0);
...
this.current_XP = 0;
this.next_XP = 1;
this.dmg_Taken = false;


this.max_hp = 1 * this.level ;
this.hp_now = 1 * this.level;


this.newDisplay = instance_create(this.x, this.y - 25, obj_hp_display);
this.newDisplay.max_hp = this.max_hp;
this.newDisplay.hp_now = this.hp_now;

this.newLvlDisplay = instance_create(this.x, this.y - 25, obj_level_display);
this.newLvlDisplay.level = this.level;
this.newLvlDisplay.experience = this.current_XP;
}
};
...
if ( keyboard_check_pressed( vk_space )) {
	new_bomb = instance_create( x + .1, y, obj_bomb );
}

if ( keyboard_check_pressed( vk_1 )) {
	new_bomb = instance_create( x + .1, y, obj_Lightning );
}
//Update the position of the players level and experience
this.newLvlDisplay.x = x-16
this.newLvlDisplay.y = y-30
//Update the hp display position
this.newDisplay.x = x-16
this.newDisplay.y = y-40

//Update the hpdaisplay
this.newDisplay.max_hp = this.max_hp;
this.newDisplay.hp_now = this.hp_now;
...
//Manage
if(this.current_XP > this.next_XP ){
	this.next_XP = ( this.next_XP + 1 ) * 2;
	this.level +=1;
	new_efx = instance_create( x, y, obj_ExpShow );
	sound_play(snd_TransFormGood);
	this.max_hp = 1.7 * this.level ;
	this.hp_now = this.max_hp;
...
//Handle Death
if(this.hp_now <= 0 ){
	//Destroy the hp display
	//this.newDisplay.Died = true;
	//Destroy current instance
	//instance_destroy()
	this.hp_now = 10;
}

///Update level dislpay
this.newLvlDisplay.level = this.level;
this.newLvlDisplay.experience = this.current_XP;
...
//When player hits any experience orb

//if ( place_meeting(x,y,obj_ExpOrb) ){
	//obj_player.current_XP = obj_player.current_XP + 1;
	//--instance_destroy();
//}
}
};
...
this.other = this.place_meeting(this.x, this.y, obj_ExpOrb);
if(this.other != null) {

delta_time = 1/60.0;
// Only take damage if not already taking damage
if (this.dmg_Taken === false) {
    

	this.current_XP +=1;    // Set debounce
	//Add Knockback
	
    // Optional: play hit animation or sound here
    // this.playAnimation("hit");
    // this.playSound("damage");
}

// Simple debounce reset using delta time
if (this.dmg_Taken === true) {
    // Reduce a timer each frame
    if (this.dmgTimer === undefined) this.dmgTimer = 0.3; // Debounce duration in seconds
    this.dmgTimer -= delta_time;

    if (this.dmgTimer <= 0) {
        this.dmg_Taken = false; // Reset debounce
        this.dmgTimer = 0.6;    // Reset timer for next hit
    }
}
...
__instance_init__(this, obj_diamond, null, 1, 0, spr_diamond, 1, 18);
this.on_creation = on_creation_i;
this.on_destroy = on_destroy_i;
this.on_step = on_step_i;
this.on_end_step = on_end_step_i;
...
function __obj_bomb() {
__instance_init__(this, obj_bomb, null, 1, 0, spr_bomb, 1, 21);
this.on_creation = function() {
with(this) {
...
__instance_init__(this, obj_explosion, null, 1, 0, spr_ExplosionFx, 1, 22);
this.on_creation = function() {
with(this) {
sound_play(sound_13);
}
...
function __obj_enemy_old() {
__instance_init__(this, obj_enemy_old, null, 1, 0, spr_enemy, 1, 23);
this.on_creation = on_creation_i;
this.on_destroy = on_destroy_i;
this.on_step = function() {
with(this) {
...
this.other = this.place_meeting(this.x, this.y, obj_explosion);
if(this.other != null) {

// destroy this enemy when touches
// the explosion
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var obj_enemy_old = new __obj_enemy_old();

function __obj_enemy() {
__instance_init__(this, obj_enemy, null, 1, 0, spr_enemy, 1, 24);
this.on_creation = function() {
with(this) {
// =============================
// SIMPLE ADVANCED AI FOR TULULOO
// =============================

// Initialize AI vars (run this in the enemy's Create event)
...
with(this) {
// ============================
// Tululoo AI – Corrected Y-axis, Mini Patrol & Chase
// ============================

// ---- Initialization ----
if(typeof this.ai_state === "undefined"){
    this.ai_state = "patrol"; // patrol, wait, chase, search

    // Starting position
    this.patrol_start_x = x;
    this.patrol_start_y = y;

    // Initialize image_angle for sprite facing
    this.image_angle = 0;

    // Small 7px patrol rectangle
    var patrol_dist = 7;
    this.patrol_points = [
        {x: this.patrol_start_x - patrol_dist, y: this.patrol_start_y - patrol_dist},
        {x: this.patrol_start_x + patrol_dist, y: this.patrol_start_y - patrol_dist},
        {x: this.patrol_start_x + patrol_dist, y: this.patrol_start_y + patrol_dist},
        {x: this.patrol_start_x - patrol_dist, y: this.patrol_start_y + patrol_dist}
    ];
    ...
    // Timers
    this.ai_timer = 0;
    this.ai_speed_patrol = 1.5;
    this.ai_speed_chase = 2.5;

    // Detection
    this.ai_detect_range = 250;
    this.ai_search_time = 90;
    this.last_seen_x = x;
    this.last_seen_y = y;

    // Settings
    this.min_distance_player = 3;
}

// ---- Helpers ----
function dist(ax, ay, bx, by){ return Math.hypot(bx-ax, by-ay); }

// Check if a line is blocked by obj_block
function lineBlocked(x1, y1, x2, y2){
    var steps = 16;
    for(var i=0;i<=steps;i++){
        var t = i/steps;
...
// Move towards a point while avoiding walls
function moveTowards(tx, ty, spd, min_dist){
    if(typeof min_dist === "undefined") min_dist = 0;
    ...
    // Corrected Y-axis for Tululoo (top-down)
    var nx = x + Math.cos(rad)*spd;
    var ny = y - Math.sin(rad)*spd;
...
// Check if the player is visible (distance + walls)
function canSeePlayer(){
    var player = global.player;
    if(!player) return false;

    var d = dist(x, y, player.x, player.y);
    if(d > this.ai_detect_range) return false;

    if(lineBlocked(x, y, player.x, player.y)) return false;

    return true; // player visible
}

var seePlayer = canSeePlayer();
var player = global.player;

// ---- STATE MACHINE ----
if(this.ai_state==="patrol"){
    var target = this.patrol_points[this.patrol_index];

    // Skip blocked patrol points
    if(!lineBlocked(x, y, target.x, target.y)){
        moveTowards(target.x, target.y, this.ai_speed_patrol);
    } else {
        this.patrol_index = (this.patrol_index+1)%this.patrol_points.length;
        target = this.patrol_points[this.patrol_index];
    }

    // Sprite faces patrol target
    image_angle = point_direction(x, y, target.x, target.y);

    if(dist(x, y, target.x, target.y) < 1){
        this.patrol_index = (this.patrol_index+1) % this.patrol_points.length;
        this.ai_timer = 15 + Math.floor(Math.random()*15);
        this.ai_state = "wait";
    }

    if(seePlayer) this.ai_state="chase";
}
else if(this.ai_state==="wait"){
    this.ai_timer -=1;
    if(this.ai_timer<=0) this.ai_state="patrol";
    if(seePlayer) this.ai_state="chase";
}
else if(this.ai_state==="chase"){
    if(seePlayer){
        moveTowards(player.x, player.y, this.ai_speed_chase, this.min_distance_player);
        image_angle = point_direction(x, y, player.x, player.y);
        this.last_seen_x = player.x;
        this.last_seen_y = player.y;
    } else {
        this.ai_state="search";
        this.ai_timer = this.ai_search_time;
    }

    var dToPlayer = dist(x, y, player.x, player.y);
    if(dToPlayer > this.ai_detect_range*1.5) this.ai_state="patrol";
}
else if(this.ai_state==="search"){
    moveTowards(this.last_seen_x, this.last_seen_y, this.ai_speed_patrol);
    image_angle = point_direction(x, y, this.last_seen_x, this.last_seen_y);
    this.ai_timer -=1;

    if(this.ai_timer<=0){
        moveTowards(this.patrol_start_x, this.patrol_start_y, this.ai_speed_patrol);
        image_angle = point_direction(x, y, this.patrol_start_x, this.patrol_start_y);
        if(dist(x, y, this.patrol_start_x, this.patrol_start_y)<1){
...
function __obj_enemy_MoveAway() {
__instance_init__(this, obj_enemy_MoveAway, null, 1, 0, spr_enemy, 1, 25);
this.on_creation = function() {
with(this) {
// =============================
// SIMPLE ADVANCED AI FOR TULULOO
...
with(this) {
// ============================
// Tululoo AI – Mini Patrol & Reliable Chase
// ============================
...
if(typeof this.ai_state === "undefined"){
    ...
    // Small 7px patrol rectangle (can adjust to 10px for visibility)
    var patrol_dist = 7;
    this.patrol_points = [
        {x: this.patrol_start_x - patrol_dist, y: this.patrol_start_y - patrol_dist}, // top-left
        {x: this.patrol_start_x + patrol_dist, y: this.patrol_start_y - patrol_dist}, // top-right
        {x: this.patrol_start_x + patrol_dist, y: this.patrol_start_y + patrol_dist}, // bottom-right
        {x: this.patrol_start_x - patrol_dist, y: this.patrol_start_y + patrol_dist}  // bottom-left
    ];
    this.patrol_index = 0;
...
function moveTowards(tx, ty, spd, min_dist){
    ...
    // Only move if next position is not blocked
    if(!instance_position(nx, ny, obj_block)){
        x = nx; y = ny;
        return true;
    } else {
        // try small angle offsets if blocked
        var offsets = [15,-15,30,-30,45,-45];
        for(var i=0;i<offsets.length;i++){
            var a = (angle + offsets[i]) * Math.PI / 180;
            nx = x + Math.cos(a)*spd;
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var obj_enemy_MoveAway = new __obj_enemy_MoveAway();

function __obj_enemy_ExtControlled() {
__instance_init__(this, obj_enemy_ExtControlled, null, 1, 0, spr_purpSlime, 1, 26);
this.on_creation = function() {
with(this) {
// =============================
// SIMPLE ADVANCED AI FOR TULULOO
...
this.onDeath = function() {
	...
	//Apply some experience to the player
	//obj_player.current_XP+=1;
	//instance_create(x,y,obj_ExpOrb);
	
    //if (this.hb_outer) instance_destroy(this.hb_outer);
   // if (this.hb_inner) instance_destroy(this.hb_inner);
	// ROTATE THE IMAGE ANGLE
...
// AT THE BOMB'S POSITION
   ...
   // instance_destroy(this); // destroy the parent itself
	
};



//Give player some xp points
}
};
this.on_step = function() {
with(this) {
//Activate ai controler for this object
ai_controller(this,10)

this.newDisplay.x = this.x-16
this.newDisplay.y = this.y-30
...
if(this.hp_now <= 0 ){
	//Destroy the hp display
	this.newDisplay.Died = true;
	new_eperiencePoints = instance_create( x, y, obj_ExpOrb );
	//Destroy current instance
	instance_destroy()
}

if ( instan = place_meeting(x,y,obj_enemy_ExtControlled) ){
	if ( instan === this ){
		return 0;
	} else { 
...
if ( instann = place_meeting(x,y,obj_block) ){
	//if ( instann === this ){
		//return 0;
	//} else { 
...
// Only take damage if not already taking damage
if (this.dmg_Taken === false) {
    this.hp_now -= 1;          // Apply damage immediately
    this.dmg_Taken = true;     // Set debounce
	//Add Knockback
	x+-1.9;
	y+-1.9;
    // Optional: play hit animation or sound here
...
this.other = this.place_meeting(this.x, this.y, obj_Helper);
if(this.other != null) {
...
if (this.dmg_Taken === false) {
    this.hp_now -= 1;          // Apply damage immediately
	sound_play(snd_SlimeBite);
    this.dmg_Taken = true;     // Set debounce
	//Add Knockback
	this.x+=irandom(image_angle );
	this.y+=irandom(image_angle );
	//if ( place_meeting(x,y,obj_ExpOrb) ){
	//obj_player.current_XP = obj_player.current_XP + 1;
	//--instance_destroy();
//}
...
this.other = this.place_meeting(this.x, this.y, obj_enemy_ExtControlled);
if(this.other != null) {

}
this.other = this.place_meeting(this.x, this.y, obj_Lightning);
if(this.other != null) {

delta_time = 1/60.0;
// Only take damage if not already taking damage
if (this.dmg_Taken === false) {
    this.hp_now -= 3;          // Apply damage immediately
    this.dmg_Taken = true;     // Set debounce
	//Add Knockback
	x+-1.2;
	y+-1.2;
...
if (this.dmg_Taken === true) {
    // Reduce a timer each frame
    if (this.dmgTimer === undefined) this.dmgTimer = 0.2; // Debounce duration in seconds
    this.dmgTimer -= delta_time;
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var obj_enemy_ExtControlled = new __obj_enemy_ExtControlled();

function __obj_Helper() {
__instance_init__(this, obj_Helper, null, 1, 0, spr_MiniSlime, 1, 27);
this.on_creation = function() {
with(this) {
this.level = 2;
this.dmg_Taken = false;
...
this.on_step = function() {
with(this) {
helper_controller(this, 50)

this.newDisplay.x = x-16
this.newDisplay.y = y-30
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var obj_Helper = new __obj_Helper();

function __hb_Outer() {
__instance_init__(this, hb_Outer, null, 1, 0, hb_Outer, 0, 29);
this.on_creation = on_creation_i;
this.on_destroy = on_destroy_i;
this.on_step = on_step_i;
this.on_end_step = on_end_step_i;
...
function __hb_Inner() {
__instance_init__(this, hb_Inner, null, 1, 0, hb_Inner, 0, 30);
this.on_creation = on_creation_i;
this.on_destroy = on_destroy_i;
this.on_step = on_step_i;
this.on_end_step = on_end_step_i;
...
function __obj_hp_display() {
__instance_init__(this, obj_hp_display, null, 1, 0, null, 1, 31);
this.on_creation = function() {
with(this) {
// ✅ Create Event
this.hp_now = 10;
...
this.Died = false;
{
    // Automatically attach to the creator if not already assigned
    if (typeof this.parent === "undefined" || this.parent === null) {
        this.parent = other || self.creator || self;
    }
...
// Step Event
{
    // Follow parent if assigned
    if (this.parent && instance_exists(this.parent)) {
        this.x = this.parent.x;
        this.y = this.parent.y ;//+ this.offsetY;
    //} else if (this.parent && !instance_exists(this.parent)) {
...
//Handle if parent dies and pases death signal on
if (this.Died === true){
instance_destroy()

}
	
    draw_set_color(this.color[0], this.color[1], this.color[2]);
	...
	// x = x + 1;
    draw_text(this.x - 8, this.y, "HP: " + floor(this.hp_now) + "/" + floor(this.max_hp));


}
}
};
}; var obj_hp_display = new __obj_hp_display();

function __obj_level_display() {
__instance_init__(this, obj_level_display, null, 1, 0, null, 1, 34);
this.on_creation = function() {
with(this) {
// ✅ Create Event
this.level = 1;
...
instance_destroy()
	...
	// x = x + 1;
    draw_text(this.x - 8, this.y, "Lvl: " + floor(this.level) + "  Exp: " + floor(this.experience));


}
}
};
}; var obj_level_display = new __obj_level_display();

function __obj_ExpOrb() {
__instance_init__(this, obj_ExpOrb, null, 1, 0, spr_MiniBliue, 1, 35);
this.on_creation = on_creation_i;
this.on_destroy = function() {
with(this) {
sound_play(snd_Happy);
}
};
this.on_step = function() {
with(this) {
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var obj_ExpOrb = new __obj_ExpOrb();

function __obj_ExpShow() {
__instance_init__(this, obj_ExpShow, null, 1, 0, spr_MagRING, 0, 107);
this.on_creation = function() {
with(this) {
image_speed = 0.1;
}
...
this.on_draw = on_draw_i;
}; var obj_ExpShow = new __obj_ExpShow();

function __obj_Lightning() {
__instance_init__(this, obj_Lightning, null, 1, 0, spr_Lightning, 1, 108);
this.on_creation = function() {
with(this) {
...
this.on_draw = on_draw_i;
}; var obj_Lightning = new __obj_Lightning();
...
[{o:obj_Helper, x:380, y:240}],
[{o:obj_enemy_ExtControlled, x:170, y:450}],
[{o:obj_enemy_ExtControlled, x:600, y:450}],
[{o:obj_ExpOrb, x:300, y:120}],
[{o:obj_ExpOrb, x:460, y:120}],
[{o:obj_ExpOrb, x:450, y:310}],
[{o:obj_block, x:440, y:160}],
[{o:obj_block, x:480, y:160}],
[{o:obj_block, x:520, y:160}],
[{o:obj_block, x:600, y:160}],
[{o:obj_block, x:640, y:160}],
[{o:obj_block, x:680, y:160}],
[{o:obj_block, x:750, y:160}],
[{o:obj_block, x:800, y:160}],
[{o:obj_block, x:640, y:200}],
[{o:obj_block, x:640, y:240}],
[{o:obj_block, x:640, y:280}],
[{o:obj_block, x:640, y:320}],
[{o:obj_block, x:640, y:360}],
[{o:obj_block, x:600, y:360}],
[{o:obj_block, x:560, y:360}],
[{o:obj_block, x:520, y:360}],
[{o:obj_block, x:480, y:360}],
[{o:obj_block, x:440, y:360}],
[{o:obj_block, x:400, y:360}],
[{o:obj_block, x:480, y:400}],
[{o:obj_block, x:680, y:400}],
[{o:obj_block, x:720, y:440}],
[{o:obj_block, x:720, y:480}],
[{o:obj_block, x:680, y:520}],
[{o:obj_block, x:640, y:520}],
[{o:obj_block, x:600, y:520}],
[{o:obj_block, x:560, y:520}],
[{o:obj_block, x:520, y:520}],
[{o:obj_block, x:480, y:520}],
[{o:obj_block, x:360, y:400}],
[{o:obj_block, x:320, y:440}],
[{o:obj_block, x:160, y:320}],
[{o:obj_block, x:120, y:360}],
[{o:obj_block, x:80, y:400}],
[{o:obj_ExpOrb, x:700, y:450}],
[{o:obj_ExpOrb, x:700, y:470}],
[{o:obj_ExpOrb, x:700, y:490}],
[{o:obj_ExpOrb, x:680, y:500}],
[{o:obj_ExpOrb, x:680, y:480}],
[{o:obj_ExpOrb, x:680, y:460}],
[{o:obj_ExpOrb, x:680, y:440}],
[{o:obj_ExpOrb, x:660, y:450}],
[{o:obj_ExpOrb, x:660, y:470}],
[{o:obj_ExpOrb, x:660, y:490}],
[{o:obj_ExpOrb, x:490, y:120}],
[{o:obj_ExpOrb, x:530, y:120}],
[{o:obj_ExpOrb, x:600, y:120}],
[{o:obj_ExpOrb, x:650, y:130}],
[{o:obj_ExpOrb, x:690, y:120}],
[{o:obj_ExpOrb, x:760, y:120}],
[{o:obj_ExpOrb, x:810, y:120}],
[{o:obj_ExpOrb, x:130, y:320}],
[{o:obj_ExpOrb, x:100, y:340}],
[{o:obj_ExpOrb, x:70, y:370}],
[{o:obj_ExpOrb, x:370, y:450}],
[{o:obj_ExpOrb, x:410, y:420}],
[{o:obj_enemy_ExtControlled, x:740, y:340}],
[{o:obj_enemy_ExtControlled, x:880, y:460}],
[{o:obj_enemy_ExtControlled, x:610, y:320}],
[{o:obj_enemy_ExtControlled, x:490, y:240}],
[{o:obj_enemy_ExtControlled, x:450, y:450}],
[{o:obj_enemy_ExtControlled, x:440, y:540}],
[{o:obj_enemy_ExtControlled, x:240, y:350}],
[{o:obj_enemy_ExtControlled, x:800, y:430}],
[{o:obj_enemy_ExtControlled, x:830, y:360}],
[{o:obj_enemy_ExtControlled, x:840, y:350}],
[{o:obj_enemy_ExtControlled, x:830, y:300}],
[{o:obj_enemy_ExtControlled, x:750, y:270}],
[{o:obj_block, x:60, y:690}],
[{o:obj_block, x:100, y:690}],
[{o:obj_block, x:140, y:690}],
[{o:obj_block, x:180, y:690}],
[{o:obj_block, x:220, y:690}],
[{o:obj_block, x:260, y:690}],
[{o:obj_block, x:300, y:690}],
[{o:obj_block, x:340, y:680}],
[{o:obj_block, x:380, y:660}],
[{o:obj_block, x:420, y:680}],
[{o:obj_block, x:460, y:700}],
[{o:obj_block, x:500, y:660}],
[{o:obj_block, x:500, y:700}],
[{o:obj_block, x:520, y:660}],
[{o:obj_block, x:560, y:640}],
[{o:obj_block, x:380, y:760}],
[{o:obj_block, x:380, y:800}],
[{o:obj_block, x:380, y:840}],
[{o:obj_block, x:380, y:880}],
[{o:obj_block, x:340, y:880}],
[{o:obj_block, x:60, y:720}],
[{o:obj_block, x:60, y:740}],
[{o:obj_block, x:60, y:780}],
[{o:obj_block, x:60, y:820}],
[{o:obj_block, x:60, y:860}],
[{o:obj_block, x:100, y:880}],
[{o:obj_block, x:140, y:880}],
[{o:obj_block, x:180, y:880}],
[{o:obj_block, x:220, y:880}],
[{o:obj_block, x:260, y:880}],
[{o:obj_block, x:300, y:880}],
[{o:obj_enemy_ExtControlled, x:120, y:760}],
[{o:obj_enemy_ExtControlled, x:120, y:780}],
[{o:obj_enemy_ExtControlled, x:120, y:800}],
[{o:obj_enemy_ExtControlled, x:120, y:820}],
[{o:obj_enemy_ExtControlled, x:120, y:840}],
[{o:obj_enemy_ExtControlled, x:120, y:840}],
[{o:obj_enemy_ExtControlled, x:120, y:860}],
[{o:obj_enemy_ExtControlled, x:160, y:860}],
[{o:obj_enemy_ExtControlled, x:160, y:820}],
[{o:obj_enemy_ExtControlled, x:160, y:800}],
[{o:obj_enemy_ExtControlled, x:160, y:760}],
[{o:obj_enemy_ExtControlled, x:200, y:780}],
[{o:obj_enemy_ExtControlled, x:200, y:780}],
[{o:obj_enemy_ExtControlled, x:200, y:800}],
[{o:obj_enemy_ExtControlled, x:200, y:820}],
[{o:obj_enemy_ExtControlled, x:200, y:840}],
[{o:obj_enemy_ExtControlled, x:260, y:820}],
[{o:obj_enemy_ExtControlled, x:260, y:800}],
[{o:obj_enemy_ExtControlled, x:340, y:780}],
[{o:obj_enemy_ExtControlled, x:340, y:820}],
[{o:obj_enemy_ExtControlled, x:300, y:800}],
[{o:obj_ExpOrb, x:240, y:740}],
[{o:obj_ExpOrb, x:260, y:740}],
[{o:obj_ExpOrb, x:280, y:740}],
[{o:obj_ExpOrb, x:320, y:740}],
[{o:obj_ExpOrb, x:360, y:740}],
[{o:obj_ExpOrb, x:380, y:740}],
[{o:obj_ExpOrb, x:440, y:740}],
[{o:obj_ExpOrb, x:480, y:780}],
[{o:obj_ExpOrb, x:480, y:780}],
[{o:obj_ExpOrb, x:540, y:780}],
[{o:obj_ExpOrb, x:580, y:760}],
[{o:obj_ExpOrb, x:620, y:720}],
[{o:obj_ExpOrb, x:640, y:640}],
[{o:obj_ExpOrb, x:600, y:620}],
[{o:obj_ExpOrb, x:520, y:600}],
[{o:obj_ExpOrb, x:340, y:840}],
[{o:obj_ExpOrb, x:320, y:840}],
[{o:obj_ExpOrb, x:320, y:840}],
[{o:obj_ExpOrb, x:260, y:860}],
[{o:obj_ExpOrb, x:220, y:860}],
[{o:obj_ExpOrb, x:300, y:860}],
[{o:obj_ExpOrb, x:360, y:860}],
[{o:obj_ExpOrb, x:220, y:760}],
[{o:obj_ExpOrb, x:240, y:800}],
[{o:obj_ExpOrb, x:220, y:820}],
[{o:obj_ExpOrb, x:140, y:820}],
[{o:obj_ExpOrb, x:140, y:800}],
[{o:obj_ExpOrb, x:140, y:760}],
[{o:obj_ExpOrb, x:140, y:840}]];
this.start = function() {
__room_start__(this, test_level, 1024, 1024, 60, 46, 90, 65, null, 0, 0, 0, 600, 400, obj_player, 250, 250);
};
}
...
// Global helper array
if (!global.helpers) global.helpers = [];

/***********************************************************************
 * CUSTOM GLOBAL FUNCTIONS
 ***********************************************************************/
// Add a helper to the array
function addHelper(helperObj) {
    if (global.helpers.indexOf(helperObj) === -1) {
        global.helpers.push(helperObj);
    }
}

function removeHelper(helperObj) {
    var idx = global.helpers.indexOf(helperObj);
    if (idx !== -1) global.helpers.splice(idx, 1);
}
